...
export default function ServiceIndustryAreaClient({
  ...
  const siaUrl = (indSlug: string, svcSlug: string, areaSlug: string) =>
    `/industries/${indSlug}/${svcSlug}/${areaSlug}`;
      ...
      <section className="relative pt-32 pb-20 bg-slate-900 overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <nav className="flex flex-wrap items-center gap-1.5 text-xs text-white/40 mb-8">
            ...
            <span>/</span>
            <Link href={`/industries/${industry.slug}`} className="hover:text-white/70 transition-colors">{industry.badge}</Link>
            <span>/</span>
          ...
          <div className="grid lg:grid-cols-[1fr_340px] gap-12">
            ...
            <div className="prose prose-slate max-w-none">
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.15 }} variants={fadeUp} transition={{ duration: 0.5 }}>
                ...
                <p className="text-slate-600 leading-relaxed mb-6">
                  We build {sn} campaigns for {lb} businesses that target the exact terms your customers search: &ldquo;{lb} near {area.name},&rdquo; &ldquo;best {lb} in {area.name},&rdquo; and dozens of long-tail variations that drive high-intent traffic. Combined with <L href="/services/google-business-profile-management-nyc">Google Business Profile optimization</L>, <L href={`/areas/${area.slug}`}>hyper-local content for {area.name}</L>, and technical SEO that search engines love, we create a compounding growth engine for your {lb} business.
                </p>
              </motion.div>
              ...
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.15 }} variants={fadeUp} transition={{ duration: 0.5, delay: 0.1 }}>
                ...
                <p className="text-slate-600 leading-relaxed mb-6">
                  We track search volume, competitor rankings, and market trends for {lb} businesses across every neighborhood we serve — from <L href={`/areas/${area.slug}`}>{area.name}</L> to {siblingAreas.length > 0 && <><L href={`/areas/${siblingAreas[0].slug}`}>{siblingAreas[0].name}</L>{siblingAreas.length > 1 && <> and <L href={`/areas/${siblingAreas[1].slug}`}>{siblingAreas[1].name}</L></>}</>}. This granular data informs every decision we make, from keyword targeting to content strategy to link building.
                </p>
                <p className="text-slate-600 leading-relaxed mb-6">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-12">
            ...
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ duration: 0.5, delay: 0.1 }}>
              ...
              <div className="mt-8">
                <Link href={`/areas/${area.slug}`} className="text-teal-400 hover:text-teal-300 text-sm font-semibold underline underline-offset-2 font-cta">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
            {[
              ...
              { label: `${industry.badge} Marketing`, href: `/industries/${industry.slug}` },
              { label: `Marketing in ${area.name}`, href: `/areas/${area.slug}` },
