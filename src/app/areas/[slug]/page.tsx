...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    ...
    description: `Marketing company serving ${area.name}, ${regionLabel}. Local SEO starting at $950/month, custom websites from $4,600, branding, AI automation, and Google Business Profile optimization for ${area.name} businesses.`,
    alternates: { canonical: `https://consortiumnyc.com/areas/${area.slug}` },
    openGraph: {
      title: `${area.name} Marketing Company | Consortium NYC`,
      description: `Local SEO, web design, and branding for ${area.name} businesses. $950/month SEO. $4,600 websites.`,
      url: `https://consortiumnyc.com/areas/${area.slug}`,
    },
  };
}
...
export default async function AreaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const pageUrl = `https://consortiumnyc.com/areas/${area.slug}`;
  const breadcrumbs = [
